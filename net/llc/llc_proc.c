...
static int llc_seq_socket_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "@%02X %8d %8d %2d %3d %4d\n", llc->daddr.lsap,
		   ...
		   sk->sk_state,
		   sk->sk_socket ? tx_cache_get_inode(SOCK_INODE(sk->sk_socket))->i_uid : -1,
		   llc->link);
out:
	return 0;
}
...
static int llc_seq_socket_open(struct _inode *inode, struct file *file)
{
	return seq_open(file, &llc_seq_socket_ops);
}

static int llc_seq_core_open(struct _inode *inode, struct file *file)
{
