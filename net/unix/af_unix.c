...
static struct sock *unix_find_socket_byinode(struct _inode *i)
{
	struct sock *s;
	struct hlist_node *node;
	...
	sk_for_each(s, node,
		...
		if(dentry && dentry_get_inode(dentry) == i)
		{
			sock_hold(s);
			goto found;
		}
				    ...
				    int type, unsigned hash, int *error)
{
	...
	if (sunname->sun_path[0]) {
		...
		err = vfs_permission(inode, &nd, MAY_WRITE);
		if (err)
			goto put_fail;
		...
		if (u) {
			...
			if (dentry)
				touch_atime(unix_sk(u)->mnt, tx_cache_get_dentry(dentry));
		} else
			goto fail;
	}
	return u;
...
static int unix_bind(struct socket *sock, struct sockaddr *uaddr, int addr_len)
{
	...
	if (sunaddr->sun_path[0]) {
		 ...
		 */
		mode = S_IFSOCK |
			(tx_cache_get_inode(SOCK_INODE(sock))->i_mode & ~current->fs->umask);
		inode = d_get_inode(nd.dentry);
		err = vfs_mknod(inode, dentry, mode, 0);
		if (err)
			goto out_mknod_dput;
	...
	if (!sunaddr->sun_path[0]) {
		...
		list = &unix_socket_table[d_get_inode(dentry)->i_ino & (UNIX_HASH_SIZE-1)];
		u->dentry = parent(nd.dentry);
		u->mnt    = nd.mnt;
	}
...
static int unix_seq_open(struct _inode *inode, struct file *file)
{
	struct seq_file *seq;
	int rc = -ENOMEM;
