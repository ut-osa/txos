...
int sock_i_uid(struct sock *sk)
{
	...
	uid = sk->sk_socket ? tx_cache_get_inode(SOCK_INODE(sk->sk_socket))->i_uid : 0;
...
unsigned long sock_i_ino(struct sock *sk)
{
	...
	read_lock(&sk->sk_callback_lock);
	ino = sk->sk_socket ? tx_cache_get_inode(SOCK_INODE(sk->sk_socket))->i_ino : 0;
	read_unlock(&sk->sk_callback_lock);
	return ino;
}
...
static int proto_seq_open(struct _inode *inode, struct file *file)
{
