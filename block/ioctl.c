				...
				unsigned cmd, unsigned long arg)
{
	...
	case BLKGETSIZE:
		if ((tx_cache_get_inode(bdev->bd_inode)->i_size >> 9) > ~0UL)
			return -EFBIG;
		return put_ulong(arg, tx_cache_get_inode(bdev->bd_inode)->i_size >> 9);
	case BLKGETSIZE64:
		return put_u64(arg, tx_cache_get_inode(bdev->bd_inode)->i_size);
	case BLKTRACESTART:
	case BLKTRACESTOP:
	case BLKTRACESETUP:
...
{
	struct block_device *bdev = (file_get_dentry(file))->d_inode->i_bdev;
	struct gendisk *disk = bdev->bd_disk;
	int ret = -ENOIOCTLCMD;
