...
#endif
	task_thread_info(p)->preempt_count2 = 1;
	/*
...
#ifdef CONFIG_OS_VISIBILITY
	if(!mm || !mm->arg_end){
	  OSA_SCHED( NULL, next->pid, 0);
	} else {
	  OSA_SCHED( mm->arg_start, next->pid, mm->arg_end - mm->arg_start);
	}
#endif

    // Call this with current stack & next stack
    // This is similiar in function to OSA_SCHED, but it is part of osatxm
#ifdef CONFIG_TX_USER_HTM
    {
       void *esp;
       asm volatile ("movl %%esp,%0; " : "=r" (esp));
       OSA_XSETPID(next->pid, esp, next->thread.esp);
    }
#endif

#ifdef CONFIG_TX_KSTM_ASSERTIONS
	kstm_stats_sched(prev, next);
#endif
...
#endif
	task_thread_info(idle)->preempt_count2 = (idle->lock_depth >= 0);
}
