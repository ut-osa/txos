...
static ssize_t do_read(struct file *fp, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if ((int)count < sizeof(apm_event_t))
		return -EINVAL;
	if ((queue_empty(as)) && (tx_cache_get_file(fp)->f_flags & O_NONBLOCK))
		return -EAGAIN;
...
static int do_release(struct _inode * inode, struct file * filp)
{
	struct apm_user *	as;
...
static int do_open(struct _inode * inode, struct file * filp)
{
	struct apm_user *	as;
	struct _file *_filp;
	...
	as->writer = (_filp->f_mode & FMODE_WRITE) == FMODE_WRITE;
	as->reader = (_filp->f_mode & FMODE_READ) == FMODE_READ;
	spin_lock(&user_list_lock);
	as->next = user_list;
	user_list = as;
	spin_unlock(&user_list_lock);
...
static int proc_apm_open(struct _inode *inode, struct file *file)
{
