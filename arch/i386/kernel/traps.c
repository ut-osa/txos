...
void die(const char * str, struct pt_regs * regs, long err)
{
	...
	if (++die.lock_owner_depth < 3) {
		...
		/* KSTM debug statements */
		printk(KERN_EMERG "Tx Status: %d, syscall: %lu\n", atomic_read(&current->transaction->status), current->eax);
#endif
        ...
        /* cjr 8-10-06:
	 * --------------------
	 * Growing tired of always setting breakpoints
	 * on die(). Makes alot more sense to just
	 * break into the debugger as a matter of course. 
         *
         * hany 10-23-06
         * Moved the OSA_BREAKSIM down after the printing out of various debug information
         * but before panic() calls
	 */
#ifdef CONFIG_TX
	OSA_MAGIC( OSA_BREAKSIM );
	panic( "Always stop on a bug" );
#endif
