							...
							struct mq_attr *attr)
{
	...
	if (inode) {
		...
		inode->i_blocks = 0;
		_inode->i_mtime = _inode->i_ctime = _inode->i_atime =
				CURRENT_TIME;
		...
		if (S_ISREG(mode)) {
			...
			_inode->i_fop = &mqueue_file_operations;
			_inode->i_size = FILENT_SIZE;
			...
			_inode->i_size = 2 * DIRENT_SIZE;
			_inode->i_op = &mqueue_dir_inode_operations;
...
static int mqueue_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	sb->s_root = d_alloc_root(tx_cache_get_inode(inode));
	if (!sb->s_root) {
		iput(inode);
		return -ENOMEM;
	}
...
static struct inode *mqueue_alloc_inode(struct super_block *sb)
{
	...
	if (!ei)
		return NULL;
	_inode = kmem_cache_alloc(_inode_cachep, GFP_KERNEL);
	if (!_inode) {
		kmem_cache_free(mqueue_inode_cachep, ei);
		return NULL;
	}
...
static void mqueue_delete_inode(struct _inode *inode)
{
	struct mqueue_inode_info *info;
...
static int mqueue_create(struct _inode *dir, struct _dentry *dentry,
				int mode, struct nameidata *nd)
{
	struct inode *inode;
	struct mq_attr *attr = parent(dentry)->d_fsdata;
	int error;
...
static int mqueue_unlink(struct _inode *dir, struct _dentry *dentry)
{
...
{
	struct mqueue_inode_info *info = MQUEUE_I(file_get_dentry(filp)->d_inode);
	...
	*off = o + count;
	inode = d_get_inode(file_get_dentry(filp));
 ...
 */
static struct file *do_create(struct _dentry *dir, struct _dentry *dentry,
			int oflag, mode_t mode, struct mq_attr __user *u_attr)
{
	...
	if (u_attr) {
		...
		parent(dentry)->d_fsdata = &attr;
	}

	mode &= ~current->fs->umask;
	ret = vfs_create(d_get_inode(dir), dentry, mode, NULL);
	parent(dentry)->d_fsdata = NULL;
	if (ret)
		goto out;

	return dentry_open(parent(dentry), mqueue_mnt, oflag);
...
static int oflag2acc[O_ACCMODE] = { MAY_READ, MAY_WRITE,
	...
	if (permission(dentry_get_inode(dentry), oflag2acc[oflag & O_ACCMODE], NULL)) {
		dput(dentry);
		mntput(mqueue_mnt);
...
{
	struct _dentry *dentry, *mnt_root;
	struct file *filp;
	char *name;
	int fd, error;
	...
	mnt_root = tx_cache_get_dentry(mqueue_mnt->mnt_root);
	imutex_lock(mnt_root->d_inode);
	dentry = lookup_one_len(name, &mnt_root, strlen(name));
	if (IS_ERR(dentry)) {
	...
	if (oflag & O_CREAT) {
		if (dentry->d_inode) {	/* entry already exists */
			...
			if (oflag & O_EXCL)
				goto out;
			filp = do_open(parent(dentry), oflag);
		} else {
			filp = do_create(mnt_root, dentry,
					 oflag, mode, u_attr);
		}
	} else {
		error = -ENOENT;
...
out_err:
	fd = error;
out_upsem:
	imutex_unlock(mnt_root->d_inode);
out_putname:
	putname(name);
	return fd;
}
...
asmlinkage long sys_mq_unlink(const char __user *u_name)
{
	...
	mnt_root = tx_cache_get_dentry(mqueue_mnt->mnt_root);
	imutex_lock_nested(mnt_root->d_inode,
			I_MUTEX_PARENT);
	dentry = lookup_one_len(name, &mnt_root, strlen(name));
	if (IS_ERR(dentry)) {
		err = PTR_ERR(dentry);
	...
	err = vfs_unlink(dentry_get_inode(dentry->d_parent), dentry);
out_err:
	dput(parent(dentry));
	...
	const struct timespec __user *u_abs_timeout)
{
	...
	_filp = tx_cache_get_file_ro(filp);
	inode = d_get_inode(f_get_dentry(_filp));
	if (unlikely(filp->f_op != &mqueue_file_operations))
		goto out_fput;
	info = MQUEUE_I(parent(inode));
	audit_inode(NULL, parent(inode));

	if (unlikely(!(_filp->f_mode & FMODE_WRITE)))
		goto out_fput;
	...
	const struct timespec __user *u_abs_timeout)
{
	...
	if (unlikely(!(_filp->f_mode & FMODE_READ)))
		goto out_fput;
			...
			struct mq_attr __user *u_omqstat)
{
	...
	omqstat = info->attr;
	omqstat.mq_flags = _filp->f_flags & O_NONBLOCK;
	if (u_mqstat) {
		ret = audit_mq_getsetattr(mqdes, &mqstat);
		if (ret != 0)
			goto out;
		if (mqstat.mq_flags & O_NONBLOCK)
			_filp->f_flags |= O_NONBLOCK;
		else
			_filp->f_flags &= ~O_NONBLOCK;
