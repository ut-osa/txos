...
static void shm_destroy(struct ipc_namespace *ns, struct shmid_kernel *shp)
{
	...
	if (!is_file_hugepages(shp->shm_file))
		...
		user_shm_unlock(d_get_inode(file_get_dentry(shp->shm_file))->i_size,
						shp->mlock_user);
	fput (shp->shm_file);
...
static int shm_release(struct _inode *ino, struct file *file)
{
	struct shm_file_data *sfd = shm_file_data(file);
...
static int shm_fsync(struct file *file, struct _dentry *dentry, int datasync)
{
	...
	if (fsync)
		ret = fsync(sfd->file, file_get_dentry(sfd->file), datasync);
	return ret;
}

static unsigned long shm_get_unmapped_area(struct file *file,
	unsigned long addr, unsigned long len, unsigned long pgoff,
					   unsigned long flags, unsigned long trash)
{
	struct shm_file_data *sfd = shm_file_data(file);
	return get_unmapped_area(sfd->file, addr, len, pgoff, flags, 0);
}
...
static int newseg (struct ipc_namespace *ns, key_t key, int shmflg, size_t size)
{
	 ...
	 */
	d_get_inode(file_get_dentry(file))->i_ino = shp->id;
		...
		unsigned long *swp)
{
	...
	*swp = 0;
	...
	for (i = 0; i <= shm_ids(ns).max_id; i++) {
		...
		inode = d_get_inode(file_get_dentry(shp->shm_file));
...
long do_shmat(int shmid, char __user *shmaddr, int shmflg, ulong *raddr)
{
	...
	_shmfile = tx_cache_get_file_ro(shp->shm_file);
	path.dentry = dget(_shmfile->f_path.dentry);
	path.mnt    = mntget(_shmfile->f_path.mnt);
	shp->shm_nattch++;
	size = i_size_read(dentry_get_inode(path.dentry));
	shm_unlock(shp);
...
asmlinkage long sys_shmdt(char __user *shmaddr)
{
	...
	while (vma) {
		...
		if ((vma->vm_ops == &shm_vm_ops) &&
			...
			size = d_get_inode(file_get_dentry(vma->vm_file))->i_size;
