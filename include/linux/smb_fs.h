...
{
	return tx_cache_get_super_ro(sb)->s_fs_info;
}
...
{
	tx_cache_get_dentry(dentry)->d_time = jiffies - SMB_MAX_AGE(server);
}
...
{
	return (SMB_I(i)->open == server_from_inode(tx_cache_get_inode_ro(i))->generation);
}
