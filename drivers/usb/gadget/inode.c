...
ep_read (struct file *fd, char __user *buf, size_t len, loff_t *ptr)
{
	...
	if ((value = get_ready_ep (tx_cache_get_file_ro(fd)->f_flags, data)) < 0)
		return value;
...
static int
ep_release (struct _inode *inode, struct file *fd)
{
	struct ep_data		*data = fd->private_data;
	int value;
		...
		unsigned code, unsigned long value)
{
	...
	if ((status = get_ready_ep (tx_cache_get_file_ro(fd)->f_flags, data)) < 0)
		return status;
...
fail:
	...
	value = get_ready_ep(tx_cache_get_file_ro(iocb->ki_filp)->f_flags, epdata);
	if (unlikely(value < 0)) {
		kfree(priv);
		goto fail;
	}
...
static int
ep_open (struct _inode *inode, struct file *fd)
{
	struct ep_data		*data = parent(inode)->i_private;
	int			value = -EBUSY;
...
ep0_read (struct file *fd, char __user *buf, size_t len, loff_t *ptr)
{
	...
	if ((state = dev->state) == STATE_DEV_SETUP) {
		...
		if (dev->setup_in) {		/* stall IN */
			...
			if ((tx_cache_get_file_ro(fd)->f_flags & O_NONBLOCK) != 0
					&& !dev->setup_out_ready) {
				retval = -EAGAIN;
				goto done;
			}
...
scan:
	...
	if (tx_cache_get_file_ro(fd)->f_flags & O_NONBLOCK) {
		retval = -EAGAIN;
		goto done;
	}
...
static int
dev_release (struct _inode *inode, struct file *fd)
{
	struct dev_data		*dev = fd->private_data;
...
restart:
	...
	list_for_each_safe (entry, tmp, &dev->epfiles) {
		...
		dentry = ep->dentry;
		ep->dentry = NULL;
		parent = tx_cache_get_dentry(tx_cache_get_dentry(dentry)->d_parent)->d_inode;
...
static int
dev_open (struct _inode *inode, struct file *fd)
{
	struct dev_data		*dev = parent(inode)->i_private;
	int			value = -EBUSY;
		...
		int mode)
{
	...
	if (inode) {
		...
		_inode->i_mode = mode;
		_inode->i_uid = default_uid;
		_inode->i_gid = default_gid;
		inode->i_blocks = 0;
		_inode->i_atime = _inode->i_mtime = _inode->i_ctime
				= CURRENT_TIME;
		...
		struct dentry **dentry_p)
{
	...
	dentry = d_alloc_name(tx_cache_get_dentry(sb->s_root), name);
	if (!dentry)
		return NULL;
	...
	d_add (tx_cache_get_dentry(dentry), tx_cache_get_inode(inode));
	*dentry_p = dentry;
	return inode;
}
