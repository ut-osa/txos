...
static loff_t usbdev_lseek(struct _file *file, loff_t offset, int orig)
{
	loff_t ret;
 ...
 * file operations
 */
static int usbdev_open(struct _inode *inode, struct file *file)
{
...
static int usbdev_release(struct _inode *inode, struct file *file)
{
	struct dev_state *ps = file->private_data;
	struct usb_device *dev = ps->dev;
	unsigned int ifnum;
 ...
 */
static int usbdev_ioctl(struct inode *pinode, struct file *file, unsigned int cmd, unsigned long arg)
{
...
static unsigned int usbdev_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	if (tx_cache_get_file_ro(file)->f_mode & FMODE_WRITE && !list_empty(&ps->async_completed))
