...
/* Open the device */
static int os_scsi_tape_open(struct _inode * inode, struct file * filp)
{
...
static int os_scsi_tape_close(struct _inode * inode, struct file * filp)
{
	int		      result = 0;
	struct osst_tape    * STp    = filp->private_data;
