...
static int
sg_open(struct _inode *inode, struct file *filp)
{
	int dev = iminor(inode);
	int flags = tx_cache_get_file_ro(filp)->f_flags;
...
static int
sg_release(struct _inode *inode, struct file *filp)
{
	Sg_device *sdp;
	Sg_fd *sfp;
...
sg_write(struct file *filp, const char __user *buf, size_t count, loff_t * ppos)
{
	...
	if (__copy_from_user(&old_hdr, buf, SZ_SG_HEADER))
		return -EFAULT;
	blocking = !(tx_cache_get_file_ro(filp)->f_flags & O_NONBLOCK);
	if (old_hdr.reply_len < 0)
	 ...
	 unsigned int cmd_in, unsigned long arg)
{
	...
	read_only = (O_RDWR != (tx_cache_get_file_ro(filp)->f_flags & O_ACCMODE));
...
static int sg_proc_single_open_adio(struct _inode *inode, struct file *file);
...
static int sg_proc_single_open_dressz(struct _inode *inode, struct file *file);
...
static int sg_proc_single_open_version(struct _inode *inode, struct file *file);
...
static int sg_proc_single_open_devhdr(struct _inode *inode, struct file *file);
...
static int sg_proc_open_dev(struct _inode *inode, struct file *file);
...
static int sg_proc_open_devstrs(struct _inode *inode, struct file *file);
...
static int sg_proc_open_debug(struct _inode *inode, struct file *file);
