...
static int check_tape(struct scsi_tape *STp, struct file *filp)
{
	...
	unsigned short st_flags = tx_cache_get_file_ro(filp)->f_flags;
	struct st_request *SRpnt = NULL;
	...
	do_wait = ((tx_cache_get_file_ro(filp)->f_flags & O_NONBLOCK) == 0);
...
/* Open the device. Needs to be called with BKL only because of incrementing the SCSI host
   module count. */
static int st_open(struct _inode *inode, struct file *filp)
{
	int i, retval = (-EIO);
	 ...
	 */
	_filp->f_mode &= ~(FMODE_PREAD | FMODE_PWRITE);
	...
	STp->write_prot = ((_filp->f_flags & O_ACCMODE) == O_RDONLY);
...
/* Close the device and release it. BKL is not needed: this is the only thread
   accessing this tape. */
static int st_release(struct _inode *inode, struct file *filp)
{
	int result = 0;
	struct scsi_tape *STp = filp->private_data;
