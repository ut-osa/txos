...
static int default_open(struct _inode * inode, struct file * filp)
{
	if (parent(inode)->i_private)
		filp->private_data = parent(inode)->i_private;
	return 0;
}
...
int oprofilefs_create_ulong(struct super_block * sb, struct _dentry * root,
			    char const * name, unsigned long * val)
{
	struct _dentry * d = __oprofilefs_create_file(sb, root, name,
						      &ulong_fops, 0644);
	if (!d)
		return -EFAULT;
...
int oprofilefs_create_ro_ulong(struct super_block * sb, struct _dentry * root,
	char const * name, unsigned long * val)
{
	struct _dentry * d = __oprofilefs_create_file(sb, root, name,
						      &ulong_ro_fops, 0444);
...
int oprofilefs_create_ro_atomic(struct super_block * sb, struct _dentry * root,
...
int oprofilefs_create_file(struct super_block * sb, struct _dentry * root,
...
int oprofilefs_create_file_perm(struct super_block * sb, struct _dentry * root,
