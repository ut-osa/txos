...
static loff_t vcs_lseek(struct _file *file, loff_t offset, int orig)
{
	int size;

	mutex_lock(&con_buf_mtx);
	size = vcs_size(d_get_inode(f_get_dentry(file)));
	switch (orig) {
		default:
...
static int
vcs_open(struct _inode *inode, struct file *filp)
{
