...
static int rtc_proc_open(struct _inode *inode, struct file *file);
#endif
...
static int rtc_open(struct _inode *inode, struct file *file)
{
	spin_lock_irq (&rtc_lock);
...
static int rtc_release(struct _inode *inode, struct file *file)
{
#ifdef RTC_IRQ
	unsigned char tmp;
	...
	if (tx_cache_get_file_ro(file)->f_flags & FASYNC) {
		rtc_fasync (-1, file, 0);
	}
no_irq:
#endif
