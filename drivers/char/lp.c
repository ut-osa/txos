...
{
	unsigned int minor = iminor(d_get_inode(file_get_dentry(file)));
	struct parport *port = lp_table[minor].dev->port;
	char *kbuf = lp_table[minor].lp_buffer;
	ssize_t retv = 0;
	ssize_t written;
	size_t copy_size = count;
	int nonblock = ((tx_cache_get_file_ro(file)->f_flags & O_NONBLOCK) ||
			(LP_F(minor) & LP_ABORT));
...
static int lp_open(struct _inode * inode, struct file * file)
{
	...
	if ((LP_F(minor) & LP_ABORTOPEN) && !(tx_cache_get_file_ro(file)->f_flags & O_NONBLOCK)) {
		int status;
		lp_claim_parport_or_block (&lp_table[minor]);
		status = r_str(minor);
		lp_release_parport (&lp_table[minor]);
...
static int lp_release(struct _inode * inode, struct file * file)
{
	unsigned int minor = iminor(inode);
...
{
	unsigned int minor = iminor(tx_cache_get_inode_ro(inode));
	int status;
	int retval = 0;
