...
static int rng_dev_open(struct _inode *inode, struct file *filp)
{
	/* enforce read-only access to this chrdev */
	if ((tx_cache_get_file_ro(filp)->f_mode & FMODE_READ) == 0)
		return -EINVAL;
			    ...
			    size_t size, loff_t *offp)
{
	...
	while (size) {
		...
		err = -EAGAIN;
		if (!bytes_read && (tx_cache_get_file_ro(filp)->f_flags & O_NONBLOCK))
			goto out;
