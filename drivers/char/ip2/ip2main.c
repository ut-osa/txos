...
static int ip2_ipl_open(struct _inode *, struct file *);
...
ip2_open( PTTY tty, struct file *pFile )
{
	 ...
	 */
	if ( (tx_cache_get_file_ro(pFile)->f_flags & O_NONBLOCK) || (tty->flags & (1<<TTY_IO_ERROR) )) {
...
{
	unsigned int minor = iminor(d_get_inode(file_get_dentry(pFile)));
	int rc = 0;
...
{
	unsigned int iplminor = iminor(tx_cache_get_inode_ro(pInode));
	int rc = 0;
	void __user *argp = (void __user *)arg;
...
/******************************************************************************/
static int
ip2_ipl_open( struct _inode *pInode, struct file *pFile )
{
