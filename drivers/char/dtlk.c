...
static int dtlk_open(struct _inode *, struct file *);
static int dtlk_release(struct _inode *, struct file *);
			  ...
			  size_t count, loff_t * ppos)
{
	...
	if (iminor(d_get_inode(file_get_dentry(file))) != DTLK_MINOR)
		return -EINVAL;
...
static int dtlk_open(struct _inode *inode, struct file *file)
{
...
static int dtlk_release(struct _inode *inode, struct file *file)
{
