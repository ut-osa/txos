...
static loff_t nvram_llseek(struct _file *file,loff_t offset, int origin )
{
	lock_kernel();
	switch (origin) {
	case 0:
...
static int
nvram_open(struct _inode *inode, struct file *file)
{
	struct _file *_file = tx_cache_get_file_ro(file);
	spin_lock(&nvram_state_lock);

	if ((nvram_open_cnt && (_file->f_flags & O_EXCL)) ||
	    (nvram_open_mode & NVRAM_EXCL) ||
	    ((_file->f_mode & 2) && (nvram_open_mode & NVRAM_WRITE))) {
		spin_unlock(&nvram_state_lock);
		return -EBUSY;
	}

	if (_file->f_flags & O_EXCL)
		nvram_open_mode |= NVRAM_EXCL;
	if (_file->f_mode & 2)
...
static int
nvram_release(struct _inode *inode, struct file *file)
{
	spin_lock(&nvram_state_lock);
