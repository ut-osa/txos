...
static int
tipar_open(struct _inode *inode, struct file *file)
{
...
static int
tipar_close(struct _inode *inode, struct file *file)
{
	unsigned int minor = iminor(inode) - TIPAR_MINOR;
...
tipar_write (struct file *file, const char __user *buf, size_t count,
		loff_t * ppos)
{
	unsigned int minor = iminor(d_get_inode(file_get_dentry(file))) - TIPAR_MINOR;
	ssize_t n;
