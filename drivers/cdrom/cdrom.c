 ...
 * this way.
 */
int cdrom_open(struct cdrom_device_info *cdi, struct _inode *ip, struct file *fp)
{
	int ret;
	...
	cdi->use_count++;
	if ((tx_cache_get_file_ro(fp)->f_flags & O_NONBLOCK) && (cdi->options & CDO_USE_FFLAGS)) {
		...
		if (tx_cache_get_file_ro(fp)->f_mode & FMODE_WRITE) {
			ret = -EROFS;
	...
	check_disk_change(parent(ip)->i_bdev);
	return 0;
err_release:
	cdi->ops->release(cdi);
err:
...
int cdrom_release(struct cdrom_device_info *cdi, struct file *fp)
{
	...
	opened_for_data = !(cdi->options & CDO_USE_FFLAGS) ||
		!(fp && tx_cache_get_file_ro(fp)->f_flags & O_NONBLOCK);
