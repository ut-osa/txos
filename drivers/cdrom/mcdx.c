...
static int mcdx_block_open(struct _inode *inode, struct file *file)
{
	struct s_drive_stuff *p = parent(inode)->i_bdev->bd_disk->private_data;
	return cdrom_open(&p->info, inode, file);
}

static int mcdx_block_release(struct _inode *inode, struct file *file)
{
	struct s_drive_stuff *p = parent(inode)->i_bdev->bd_disk->private_data;
