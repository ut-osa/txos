...
static int idedisk_open(struct _inode *inode, struct file *filp)
{
	...
	if (drive->removable && idkp->openers == 1) {
		...
		args.command_type = IDE_DRIVE_TASK_NO_DATA;
		args.handler	  = &task_no_data_intr;
		check_disk_change(parent(inode)->i_bdev);
		/*
...
static int idedisk_release(struct _inode *inode, struct file *filp)
{
	struct gendisk *disk = parent(inode)->i_bdev->bd_disk;
