...
static int idefloppy_open(struct _inode *inode, struct file *filp)
{
	...
	if (floppy->openers == 1) {
		...
		if (idefloppy_get_capacity (drive)
		    && (tx_cache_get_file_ro(filp)->f_flags & O_NDELAY) == 0
		    /*
		...
		if (floppy->wp && (tx_cache_get_file_ro(filp)->f_mode & 2)) {
			ret = -EROFS;
			goto out_put_floppy;
		}
...
static int idefloppy_release(struct _inode *inode, struct file *filp)
{
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case IDEFLOPPY_IOCTL_FORMAT_START:
		...
		if (!(tx_cache_get_file_ro(file)->f_mode & 2))
			return -EPERM;
