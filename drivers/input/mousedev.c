...
static int mousedev_release(struct _inode *inode, struct file *file)
{
	struct mousedev_client *client = file->private_data;
...
static int mousedev_open(struct _inode *inode, struct file *file)
{
	struct mousedev_client *client;
	struct mousedev *mousedev;
	int error;
...
static ssize_t mousedev_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	_filp = tx_cache_get_file(file);

	if (!client->ready && !client->buffer && (_filp->f_flags & O_NONBLOCK))
		return -EAGAIN;
