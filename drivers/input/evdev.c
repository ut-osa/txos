...
static int evdev_release(struct _inode *inode, struct file *file)
{
	struct evdev_client *client = file->private_data;
	struct evdev *evdev = client->evdev;
...
static int evdev_open(struct _inode *inode, struct file *file)
{
	struct evdev_client *client;
	struct evdev *evdev;
...
static ssize_t evdev_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (client->head == client->tail && evdev->exist && (tx_cache_get_file_ro(file)->f_flags & O_NONBLOCK))
		return -EAGAIN;
