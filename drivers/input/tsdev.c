...
static int tsdev_open(struct _inode *inode, struct file *file)
{
...
static int tsdev_release(struct _inode *inode, struct file *file)
{
	struct tsdev_client *client = file->private_data;
			  ...
			  loff_t *ppos)
{
	...
	if (client->head == client->tail && tsdev->exist && (tx_cache_get_file_ro(file)->f_flags & O_NONBLOCK))
		return -EAGAIN;
