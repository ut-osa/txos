...
static int serio_raw_open(struct _inode *inode, struct file *file)
{
	struct serio_raw *serio_raw;
...
static int serio_raw_release(struct _inode *inode, struct file *file)
{
	struct serio_raw_list *list = file->private_data;
...
static ssize_t serio_raw_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (serio_raw->head == serio_raw->tail && (tx_cache_get_file_ro(file)->f_flags & O_NONBLOCK))
		return -EAGAIN;
