...
fb_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	 unsigned long arg)
{
	int fbidx = iminor(tx_cache_get_inode_ro(inode));
	struct fb_info *info = registered_fb[fbidx];
...
static int fb_getput_cmap(struct _inode *inode, struct file *file,
...
static int fb_get_fscreeninfo(struct _inode *inode, struct file *file,
				unsigned int cmd, unsigned long arg)
{
	mm_segment_t old_fs;
	struct fb_fix_screeninfo fix;
...
{
	int fbidx = iminor(d_get_inode(file_get_dentry(file)));
	struct fb_info *info = registered_fb[fbidx];
	struct fb_ops *fb = info->fbops;
	unsigned long off;
...
static int
fb_open(struct _inode *inode, struct file *file)
{
	int fbidx = iminor(inode);
	struct fb_info *info;
	int res = 0;
...
static int 
fb_release(struct _inode *inode, struct file *file)
{
