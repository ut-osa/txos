...
static int fd_ioctl(struct inode *pinode, struct file *filp, unsigned int cmd,
		    unsigned long param)
{
...
#define IN(c,x,tag) case c: *(x) = inparam. tag ; return 0
	...
	int drive = (long)pinode->i_bdev->bd_disk->private_data;
	...
	if (_IOC_DIR(cmd) & _IOC_WRITE)
		...
		case FDCLRPRM:
			...
			UDRS->keep_data = 0;
			return invalidate_drive(pinode->i_bdev);
		case FDSETPRM:
		case FDDEFPRM:
			return set_geometry(cmd, &inparam.g,
					    drive, type, pinode->i_bdev);
		case FDGETPRM:
...
static int floppy_release(struct _inode *inode, struct file *filp)
{
	int drive = (long)parent(inode)->i_bdev->bd_disk->private_data;
 ...
 */
static int floppy_open(struct _inode *inode, struct file *filp)
{
	struct inode *pinode = parent(inode);
	int drive = (long)pinode->i_bdev->bd_disk->private_data;
	int old_dev;
	int try;
	int res = -EBUSY;
	...
	if (opened_bdev[drive] && opened_bdev[drive] != pinode->i_bdev)
		goto out2;
	...
	if (UDRS->fd_ref == -1 || (UDRS->fd_ref && (_filp->f_flags & O_EXCL)))
		goto out2;
	...
	opened_bdev[drive] = pinode->i_bdev;
	 ...
	 * protected disks */
	if ((_filp->f_mode & FMODE_WRITE) || !file_permission(filp, MAY_WRITE))
		filp->private_data = (void *)8;

	if (UFDCS->rawcmd == 1)
		UFDCS->rawcmd = 2;

	if (!(_filp->f_flags & O_NDELAY)) {
		if (_filp->f_mode & 3) {
			UDRS->last_checked = 0;
			check_disk_change(pinode->i_bdev);
			if (UTESTF(FD_DISK_CHANGED))
				goto out;
		}
		res = -EROFS;
		if ((_filp->f_mode & 2) && !(UTESTF(FD_DISK_WRITABLE)))
			goto out;
	}
	mutex_unlock(&open_lock);
	return 0;
