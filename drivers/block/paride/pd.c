...
static int pd_open(struct _inode *inode, struct file *file)
{
	struct pd_unit *disk = parent(inode)->i_bdev->bd_disk->private_data;
...
static int pd_ioctl(struct inode *pinode, struct file *file,
	 unsigned int cmd, unsigned long arg)
{
	struct pd_unit *disk = pinode->i_bdev->bd_disk->private_data;
...
static int pd_release(struct _inode *inode, struct file *file)
{
	struct pd_unit *disk = parent(inode)->i_bdev->bd_disk->private_data;
