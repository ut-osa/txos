...
static int pt_open(struct _inode *inode, struct file *file);
static int pt_ioctl(struct inode *inode, struct file *file,
		    unsigned int cmd, unsigned long arg);
static int pt_release(struct _inode *inode, struct file *file);
...
static int pt_open(struct _inode *inode, struct file *file)
{
	...
	err = -EROFS;
	if ((!tape->flags & PT_WRITE_OK) && (tx_cache_get_file_ro(file)->f_mode & 2))
		goto out;
...
static int
pt_release(struct _inode *inode, struct file *file)
{
	struct pt_unit *tape = file->private_data;
