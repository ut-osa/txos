...
static int pf_open(struct _inode *inode, struct file *file);
static void do_pf_request(request_queue_t * q);
static int pf_ioctl(struct inode *inode, struct file *file,
		    unsigned int cmd, unsigned long arg);
static int pf_getgeo(struct block_device *bdev, struct hd_geometry *geo);

static int pf_release(struct _inode *inode, struct file *file);
...
static int pf_open(struct _inode *inode, struct file *file)
{
	struct pf_unit *pf = parent(inode)->i_bdev->bd_disk->private_data;
	...
	if ((pf->media_status == PF_RO) && (tx_cache_get_file_ro(file)->f_mode & 2))
		return -EROFS;
...
static int pf_ioctl(struct inode *pinode, struct file *file, unsigned int cmd, unsigned long arg)
{
	struct pf_unit *pf = pinode->i_bdev->bd_disk->private_data;
