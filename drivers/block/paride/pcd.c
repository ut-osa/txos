...
static int pcd_block_open(struct _inode *inode, struct file *file)
{
	struct pcd_unit *cd = parent(inode)->i_bdev->bd_disk->private_data;
	return cdrom_open(&cd->info, inode, file);
}

static int pcd_block_release(struct _inode *inode, struct file *file)
{
	struct pcd_unit *cd = parent(inode)->i_bdev->bd_disk->private_data;
	return cdrom_release(&cd->info, file);
}

static int pcd_block_ioctl(struct inode *pinode, struct file *file,
				unsigned cmd, unsigned long arg)
{
	struct pcd_unit *cd = pinode->i_bdev->bd_disk->private_data;
	return cdrom_ioctl(file, &cd->info, pinode, cmd, arg);
}
