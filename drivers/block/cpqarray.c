...
static int ida_open(struct _inode *inode, struct file *filep);
static int ida_release(struct _inode *inode, struct file *filep);
...
static int ida_open(struct _inode *inode, struct file *filep)
{
	...
	drv_info_t *drv = get_drv(pinode->i_bdev->bd_disk);
	ctlr_info_t *host = get_host(pinode->i_bdev->bd_disk);

	DBGINFO(printk("ida_open %s\n", pinode->i_bdev->bd_disk->disk_name));
	/*
...
static int ida_release(struct _inode *inode, struct file *filep)
{
	ctlr_info_t *host = get_host(parent(inode)->i_bdev->bd_disk);
	host->usage_count--;
	return 0;
}
 ...
 */
static int ida_ioctl(struct inode *pinode, struct file *filep, unsigned int cmd, unsigned long arg)
{
