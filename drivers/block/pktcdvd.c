...
static int pkt_debugfs_fops_open(struct _inode *inode, struct file *file)
{
	return single_open(file, pkt_debugfs_seq_show, parent(inode)->i_private);
}
...
static int pkt_open(struct _inode *inode, struct file *file)
{
	...
	if (pd->refcnt > 1) {
		...
		ret = pkt_open_dev(pd, tx_cache_get_file_ro(file)->f_mode & FMODE_WRITE);
		if (ret)
			goto out_dec;
		/*
		 ...
		 */
		set_blocksize(parent(inode)->i_bdev, CD_FRAMESIZE);
	}
...
static int pkt_close(struct _inode *inode, struct file *file)
{
	struct pktcdvd_device *pd = parent(inode)->i_bdev->bd_disk->private_data;
	int ret = 0;
...
static int pkt_seq_open(struct _inode *inode, struct file *file)
{
...
static int pkt_ioctl(struct inode *pinode, struct file *file, unsigned int cmd, unsigned long arg)
{
	struct _inode *inode = tx_cache_get_inode(pinode);
	struct pktcdvd_device *pd = pinode->i_bdev->bd_disk->private_data;
