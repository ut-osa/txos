...
static int rd_ioctl(struct inode *pinode, struct file *file,
			unsigned int cmd, unsigned long arg)
{
	...
	if (bdev->bd_openers <= 2) {
		truncate_inode_pages(tx_cache_get_inode(bdev->bd_inode)->i_mapping, 0);
		error = 0;
	}
...
static int rd_open(struct _inode *inode, struct file *filp)
{
	unsigned unit = iminor(inode);

	if (rd_bdev[unit] == NULL) {
		struct block_device *bdev = parent(inode)->i_bdev;
		struct address_space *mapping;
		unsigned bsize;
		gfp_t gfp_mask;

		inode = tx_cache_get_inode((igrab(bdev->bd_inode)));
		rd_bdev[unit] = bdev;
		bdev->bd_openers++;
