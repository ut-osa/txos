...
static int cciss_open(struct _inode *inode, struct file *filep);
static int cciss_release(struct _inode *inode, struct file *filep);
...
static int cciss_open(struct _inode *inode, struct file *filep)
{
	...
	drive_info_struct *drv = get_drv(pinode->i_bdev->bd_disk);
 ...
 * ioctl
 */
static int cciss_ioctl(struct inode *pinode, struct file *filep,
		       unsigned int cmd, unsigned long arg)
{
	struct block_device *bdev = pinode->i_bdev;
