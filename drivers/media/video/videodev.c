...
{
	return video_device[iminor(d_get_inode(file_get_dentry(file)))];
}

/*
 *	Open a video device - FIXME: Obsoleted
 */
static int video_open(struct _inode *inode, struct file *file)
{
 ...
 */
int video_exclusive_open(struct _inode *inode, struct file *file)
{
...
int video_exclusive_release(struct _inode *inode, struct file *file)
{
	struct  video_device *vfl = video_devdata(file);
...
static int __video_do_ioctl(struct _inode *inode, struct file *file,
		unsigned int cmd, void *arg)
{
...
int video_ioctl2 (struct _inode *inode, struct file *file,
	       unsigned int cmd, unsigned long arg)
{
	char	sbuf[128];
	void    *mbuf = NULL;
