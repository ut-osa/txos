...
static inline struct autofs_sb_info *autofs4_sbi(struct _super_block *sb)
{
...
{
  d_get_inode(file_get_dentry(dst))->i_atime =
    d_get_inode(file_get_dentry(src))->i_atime;
	return;
}
...
{
  return dentry_get_inode_ro(sbi->sb->s_root)->i_ino;
}
