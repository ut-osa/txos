		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (path) {
		fn = d_path(file_get_dentry_ro(filp), _filp->f_path.mnt, path, PAGE_SIZE);
		if (IS_ERR(fn))
			fn = "?";
	}
				...
				unsigned long arg)
{
	...
	case FIGETBSZ:
	case FIONREAD:
		if (S_ISREG(tx_cache_get_inode_ro(file_get_dentry_ro(filp)->d_inode)->i_mode))
			break;
		/*FALL THROUGH*/
...
#ifdef CONFIG_NET
	if (S_ISSOCK(tx_cache_get_inode_ro(file_get_dentry_ro(filp)->d_inode)->i_mode) &&
