...
static int get_more_blocks(struct dio *dio)
{
	...
	int create;
	struct _inode *_inode = tx_cache_get_inode_ro(dio->inode);
	...
	if (ret == 0) {
		...
		map_bh->b_state = 0;
		map_bh->b_size = fs_count << _inode->i_blkbits;

		create = dio->rw & WRITE;
		if (dio->lock_type == DIO_LOCKING) {
			if (dio->block_in_file < (i_size_read(_inode) >>
							dio->blkbits))
				create = 0;
		 ...
		 * writes.
		 */
		ret = (*dio->get_block)(_inode, fs_startblk,
					map_bh, create);
	}
	return ret;
}
...
static void clean_blockdev_aliases(struct dio *dio)
{
	...
	nblocks = dio->map_bh.b_size >> tx_cache_get_inode(dio->inode)->i_blkbits;
...
do_holes:
			...
			if (!buffer_mapped(map_bh)) {
				 ...
				 */
				i_size_aligned = ALIGN(i_size_read(tx_cache_get_inode(dio->inode)),
							1 << blkbits);
	...
	struct dio *dio)
{
	...
	dio->rw = rw;
	dio->blkbits = blkbits;
	dio->blkfactor = _inode->i_blkbits - blkbits;
	...
	blk_run_address_space(_inode->i_mapping);
	...
	int dio_lock_type)
{
	...
	unsigned long addr;
	struct _inode *_inode = tx_cache_get_inode(inode);
	unsigned blkbits = _inode->i_blkbits;
