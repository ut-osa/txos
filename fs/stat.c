...
void generic_fillattr(const struct _inode *inode, struct kstat *stat)
{
	...
	stat->blocks = parent(inode)->i_blocks;
	stat->blksize = (1 << inode->i_blkbits);
}

EXPORT_SYMBOL(generic_fillattr);

int vfs_getattr(struct vfsmount *mnt, const struct _dentry *dentry, struct kstat *stat)
{
	const struct _inode *inode = d_get_inode_ro(dentry);
	int retval;

	if(unlikely(PTR_ERR(inode) == -ETXABORT))
		return -ETXABORT;
...
int vfs_fstat(unsigned int fd, struct kstat *stat)
{
	...
	if (f) {
		...
		error = vfs_getattr(_f->f_path.mnt, f_get_dentry_ro(_f), stat);
		fput(f);
	}
	return error;
}
