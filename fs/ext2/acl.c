 ...
 */
static struct posix_acl *
ext2_get_acl(const struct _inode *inode, int type)
{
	...
	switch(type) {
		case ACL_TYPE_ACCESS:
			acl = ext2_iget_acl(parent(inode), &ei->i_acl);
		...
		case ACL_TYPE_DEFAULT:
			acl = ext2_iget_acl(parent(inode), &ei->i_default_acl);
			if (acl != EXT2_ACL_NOT_CACHED)
				return acl;
	...
	if (!IS_ERR(acl)) {
		switch(type) {
			case ACL_TYPE_ACCESS:
				ext2_iset_acl(parent(inode), &ei->i_acl, acl);
				break;

			case ACL_TYPE_DEFAULT:
				ext2_iset_acl(parent(inode), &ei->i_default_acl, acl);
				break;
		}
	}
	return acl;
 ...
 */
static int
ext2_set_acl(struct _inode *inode, int type, struct posix_acl *acl)
{
	struct ext2_inode_info *ei = EXT2_I(parent(inode));
	int name_index;
	void *value = NULL;
...
static int
ext2_check_acl(const struct _inode *inode, int mask)
{
...
int
ext2_permission(const struct _inode *inode, int mask, struct nameidata *nd)
{
			   ...
			   const char *name, size_t name_len)
{
	...
	if (!test_opt(inode_get_sb(inode), POSIX_ACL))
		return 0;
...
static int
ext2_xattr_get_acl(struct _inode *inode, int type, void *buffer, size_t size)
{
	struct posix_acl *acl;
	int error;
...
static int
ext2_xattr_get_acl_access(struct _inode *inode, const char *name,
...
static int
ext2_xattr_get_acl_default(struct _inode *inode, const char *name,
			   void *buffer, size_t size)
{
	if (strcmp(name, "") != 0)
		return -EINVAL;
...
static int
ext2_xattr_set_acl_access(struct _inode *inode, const char *name,
...
static int
ext2_xattr_set_acl_default(struct _inode *inode, const char *name,
			   const void *value, size_t size, int flags)
{
	if (strcmp(name, "") != 0)
		return -EINVAL;
