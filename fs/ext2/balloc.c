					     ...
					     struct buffer_head ** bh)
{
	...
	group_desc = block_group >> EXT2_DESC_PER_BLOCK_BITS(_sb);
	offset = block_group & (EXT2_DESC_PER_BLOCK(_sb) - 1);
		       ...
		       unsigned long count)
{
	...
	if (block < le32_to_cpu(es->s_first_data_block) ||
		...
		printk(KERN_ERR "Current tx status= %d\n", atomic_read(&current->transaction->status));
		{
			struct ext2_inode_info *ei = EXT2_I(inode);
			printk(KERN_ERR "prealloc block at %p\n", &ei->i_prealloc_block);
...
do_more:
	 ...
	 * boundary.
	 */
	if (bit + count > EXT2_BLOCKS_PER_GROUP(_sb)) {
		overflow = bit + count - EXT2_BLOCKS_PER_GROUP(_sb);
		count -= overflow;
	}
	brelse(bitmap_bh);
			...
			u32 *prealloc_count, u32 *prealloc_block, int *err)
{
	...
	struct ext2_sb_info *sbi = EXT2_SB(_sb);
	struct ext2_super_block *es = sbi->s_es;
	unsigned group_size = EXT2_BLOCKS_PER_GROUP(_sb);
...
got_block:
	...
	mark_buffer_dirty(bitmap_bh);
	if (tx_cache_get_super_ro(sb)->s_flags & MS_SYNCHRONOUS)
		sync_dirty_buffer(bitmap_bh);
...
unsigned long ext2_count_free_blocks (struct super_block * sb)
{
	...
	return bitmap_count;
#else
        for (i = 0; i < EXT2_SB(tx_cache_get_super_ro(sb))->s_groups_count; i++) {
                desc = ext2_get_group_desc (sb, i, NULL);
                if (!desc)
                        continue;
                desc_count += le16_to_cpu(desc->bg_free_blocks_count);
...
static inline int
block_in_use(unsigned long block, struct _super_block *sb, unsigned char *map)
{
 ...
 */
int ext2_bg_has_super(struct _super_block *sb, int group)
{
 ...
 */
unsigned long ext2_bg_num_gdb(struct _super_block *sb, int group)
{
	if (EXT2_HAS_RO_COMPAT_FEATURE(sb,EXT2_FEATURE_RO_COMPAT_SPARSE_SUPER)&&
	    !ext2_group_sparse(group))
		return 0;
