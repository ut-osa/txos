		     ...
		     const char *name, size_t name_len)
{
	...
	if (!test_opt(inode_get_sb(inode), XATTR_USER))
		return 0;
...
static int
ext2_xattr_user_get(struct _inode *inode, const char *name,
		    void *buffer, size_t size)
{
	if (strcmp(name, "") == 0)
		return -EINVAL;
	if (!test_opt(i_get_sb(inode), XATTR_USER))
		return -EOPNOTSUPP;
	return ext2_xattr_get(inode, EXT2_XATTR_INDEX_USER, name, buffer, size);
}

static int
ext2_xattr_user_set(struct _inode *inode, const char *name,
