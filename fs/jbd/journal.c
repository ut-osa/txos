		 ...
		 unsigned long *retp)
{
	...
	if (journal->j_inode) {
		struct _inode *_j_inode = tx_cache_get_inode(journal->j_inode);
		ret = bmap(_j_inode, blocknr);
		if (ret)
			*retp = ret;
		else {
...
journal_t * journal_init_inode (struct inode *inode)
{
	...
	sb = _inode->i_sb;
	journal->j_dev = journal->j_fs_dev = sb->s_bdev;
	journal->j_inode = inode;
	jbd_debug(1,
		  "journal %p: inode %s/%ld, size %Ld, bits %d, blksize %ld\n",
		  journal, sb->s_id, _inode->i_ino,
		  (long long) _inode->i_size,
		  sb->s_blocksize_bits, sb->s_blocksize);

	journal->j_maxlen = _inode->i_size >> sb->s_blocksize_bits;
	journal->j_blocksize = sb->s_blocksize;
...
static const char *journal_dev_name(journal_t *journal, char *buffer)
{
	...
	if (journal->j_inode)
		bdev = tx_cache_get_inode(journal->j_inode)->i_sb->s_bdev;
	else
		bdev = journal->j_dev;
...
{
	return 1 << (PAGE_CACHE_SHIFT - tx_cache_get_inode(inode)->i_sb->s_blocksize_bits);
}
