...
static int __aio_put_req(struct kioctx *ctx, struct kiocb *req)
{
	 ...
	 */
	if (unlikely(tx_atomic_dec_and_test(&req->ki_filp->f_count))) {
		get_ioctx(ctx);
		spin_lock(&fput_lock);
...
static ssize_t aio_setup_iocb(struct kiocb *kiocb)
{
	...
	case IOCB_CMD_PREAD:
		ret = -EBADF;
		if (unlikely(!(tx_cache_get_file_ro(file)->f_mode & FMODE_READ)))
			break;
		ret = -EFAULT;
	...
	case IOCB_CMD_PWRITE:
		ret = -EBADF;
		if (unlikely(!(tx_cache_get_file_ro(file)->f_mode & FMODE_WRITE)))
			break;
		ret = -EFAULT;
