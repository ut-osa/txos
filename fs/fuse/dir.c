...
{
	_get_fuse_inode(inode)->i_time = 0;
}
 ...
 */
static int fuse_d_add_directory(struct dentry *entry, struct _inode *inode)
{
	...
	if (alias) {
		...
		dput(alias);
		if (!tx_list_empty(&inode->i_dentry))
			return -EBUSY;
	}
	d_add(entry, inode);
	return 0;
...
static int fuse_mknod(struct _inode *dir, struct dentry *entry, int mode,
		      dev_t rdev)
{
...
static int fuse_create(struct _inode *dir, struct dentry *entry, int mode,
		       struct nameidata *nd)
{
...
static int fuse_symlink(struct _inode *dir, struct dentry *entry,
			const char *link)
{
	struct fuse_conn *fc = get_fuse_conn(dir);
...
static int fuse_dir_open(struct _inode *inode, struct file *file)
{
	return fuse_open_common(inode, file, 1);
}

static int fuse_dir_release(struct _inode *inode, struct file *file)
{
