...
struct file *open_exec(const char *name)
{
	...
	if (!err) {
		...
		if (!(nd.mnt->mnt_flags & MNT_NOEXEC) &&
			...
			int err = vfs_permission(inode, &nd, MAY_EXEC);
			file = ERR_PTR(err);
			if (!err) {
				file = nameidata_to_filp(&nd, O_RDONLY);
				if (!IS_ERR(file)) {
...
{
	int mode;
	struct _inode * inode = d_get_inode(file_get_dentry(bprm->file));
	int retval;
	...
	if(!(tx_cache_get_file_ro(bprm->file)->f_path.mnt->mnt_flags & MNT_NOSUID)) {
		/* Set-uid? */
		if (mode & S_ISUID) {
...
int do_coredump(long signr, int exit_code, struct pt_regs * regs)
{
	...
	if (!ispipe && d_unhashed(file_get_dentry_ro(file)))
		goto close_fail;
	...
	if (!file->f_op->write)
		goto close_fail;
	if (!ispipe && do_truncate(file_get_dentry(file), 0, 0, file) != 0)
		goto close_fail;
