...
asmlinkage long compat_sys_statfs(const char __user *path, struct compat_statfs __user *buf)
{
	...
	if (!error) {
		struct kstatfs tmp;
		error = vfs_statfs(nd.dentry->d_parent, &tmp);
		if (!error)
...
asmlinkage long compat_sys_fstatfs(unsigned int fd, struct compat_statfs __user *buf)
{
	...
	error = -EBADF;
	file = tx_cache_get_file(fget(fd));
	if (!file)
		goto out;
...
asmlinkage long compat_sys_fstatfs64(unsigned int fd, compat_size_t sz, struct compat_statfs64 __user *buf)
{
	...
	error = -EBADF;
	_file = tx_cache_get_file(fget(fd));
	if (!_file)
		goto out;
	error = vfs_statfs(_file->f_path.dentry, &tmp);
	if (!error)
		error = put_compat_statfs64(buf, &tmp);
	fput(_file->parent);
out:
		...
		struct compat_linux_dirent __user *dirent, unsigned int count)
{
	...
	error = -EBADF;
	_file = tx_cache_get_file_ro(fget(fd));
	if (!_file)
		goto out;
	...
	error = vfs_readdir(_file->parent, compat_filldir, &buf);
	if (error < 0)
		goto out_putf;
	error = buf.error;
	lastdirent = buf.previous;
	if (lastdirent) {
		if (put_user(_file->f_pos, &lastdirent->d_off))
			error = -EFAULT;
		else
		...
		struct linux_dirent64 __user * dirent, unsigned int count)
{
	...
	lastdirent = buf.previous;
	if (lastdirent) {
		typeof(lastdirent->d_off) d_off = tx_cache_get_file_ro(file)->f_pos;
		error = -EFAULT;
...
out:
	...
	if ((ret + (type == READ)) > 0) {
		...
		if (type == READ)
			fsnotify_access(_dentry);
		else
			fsnotify_modify(_dentry->d_parent);
	}
	return ret;
}
...
compat_sys_readv(unsigned long fd, const struct compat_iovec __user *vec, unsigned long vlen)
{
	...
	ret = compat_do_readv_writev(READ, file, vec, vlen, &_file->f_pos);
...
compat_sys_writev(unsigned long fd, const struct compat_iovec __user *vec, unsigned long vlen)
{
	...
	ret = compat_do_readv_writev(WRITE, file, vec, vlen, &_file->f_pos);
